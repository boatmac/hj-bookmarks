...
- [`docs/AZURE-BLOB.md`](docs/AZURE-BLOB.md)：全球 Azure 与 Azure 中国区的 Blob、SAS、CORS 和 ETag 配置
...
- WebDAV 与 Azure Blob（全球 Azure / Azure 中国区）跨设备双向同步，远端数据使用 PBKDF2 + AES-GCM 加密
...
- **远端同步**：填写服务提供的地址及对应访问凭据；应用自动识别 WebDAV、Koofr 或 Azure Blob
- **本地同步目录**：选择由桌面同步软件管理的文件夹

向导随后确认加密口令、自动同步和刷新凭据偏好，并在完成前执行一次真实的读取、写入和加密合并验证。Koofr、Azure Blob、标准 WebDAV 及其浏览器兼容差异不会作为用户需要选择的同步类型；服务识别与协议适配封装在远端适配层中。尚未配置同步时，可以从右上角“备份/同步 → 同步设置向导”或同步设置窗口进入；连接成功后向导入口会隐藏，同步方式、远端地址和用户名会锁定。需要更换同步位置时必须先移除当前配置。

## 云端加密同步

首次使用时可以点击首页引导卡片中的“设置同步”，也可以在右上角“备份/同步”菜单中选择“加密同步”；随后在向导中选择“远端同步”，填写：

- WebDAV、Koofr、Azure Blob 容器或完整文件地址
- WebDAV / Koofr 使用的用户名（Azure Blob 不需要）
- 密码、应用密码或 Azure Blob SAS Token
- 至少 8 个字符的加密口令
...
也可以直接填写完整的 `.json` 文件地址。对于支持目录操作的 WebDAV / Koofr，默认开启“自动创建同步目录”：首次同步时会创建同步文件所在的最后一级目录；更上层路径仍需提前存在。Azure Blob 容器必须预先创建。
...
Azure Blob 适配器支持全球 Azure 的 `blob.core.windows.net` 和世纪互联运营的 Azure 中国区 `blob.core.chinacloudapi.cn`。可以填写容器或完整 Blob 地址；容器地址会自动追加固定同步文件名。应用使用 SAS Token 进行浏览器直连，并通过 ETag、`If-Match` 和 `If-None-Match` 防止并发覆盖。
...
如果粘贴完整 SAS URL，应用会在本机立即把查询参数从地址中移到密码类型的 SAS 输入框；长期设置只保存不含 SAS 的基础地址。容器必须预先存在，并通过 Blob 服务 CORS 允许 GET、HEAD、PUT 以及 ETag 等必要请求头。配置、权限和数据保护建议见 [`docs/AZURE-BLOB.md`](docs/AZURE-BLOB.md)。
...
- 不含秘密查询参数的地址、必要用户名和自动同步偏好保存在本机
- WebDAV 密码、Azure SAS Token 及加密口令默认只保留在当前页面内存中
- 可选择“刷新本标签页时保留凭据”，使用 `sessionStorage` 跨刷新恢复
- 取消选项、离开页面、关闭后重新打开或移除同步配置时清除会话凭据
- WebDAV 首次同步可自动创建最后一级目录；Azure Blob 只创建 Blob，不创建容器
- 不会递归创建更上层目录或 Azure 容器，避免误建位置
- 后续同步会合并本机和远端数据
- 开启自动同步并完成解锁后，远端模式每 60 秒检查一次共享库版本
- 页面切回前台、窗口重新获得焦点或网络恢复时会提前检查
- 标准 WebDAV 使用 ETag 条件 GET，Koofr 读取 Hash，Azure Blob 使用 HEAD 与 ETag
...
“刷新本标签页时保留凭据”默认关闭。开启后，密码、SAS Token 和加密口令只写入该标签页的 `sessionStorage`。应用还会检查 Navigation Timing：只有本次导航明确属于 `reload` 才恢复；普通重新打开、地址栏重新进入、离开后返回或浏览器会话恢复都会先清除。该选项不会把凭据写入 IndexedDB、localStorage、源码或远端文件。

### 连接兼容性

Koofr 地址会自动使用适合浏览器访问的连接方式。标准 WebDAV 和 Azure Blob 必须配置允许当前网页 Origin 的 CORS；Azure Blob 还必须向浏览器公开 ETag。连接不兼容时应用会停止写入并给出通用错误。
...
协议、CORS、加密格式、凭据边界及冲突算法详见 [`docs/SYNC-DESIGN.md`](docs/SYNC-DESIGN.md)；Azure 专项配置见 [`docs/AZURE-BLOB.md`](docs/AZURE-BLOB.md)。

## 可信小组共享书签库

家庭、工作室或小型团队可以把静态部署的 HJ Bookmarks 作为同一个加密共享书签库入口。每台设备仍使用独立 IndexedDB，但配置同一个 WebDAV 文件、Azure Blob 或其他受支持远端位置及相同加密口令；完成首次同步后，自动同步会上传本机变化，并在页面可见时定期发现其他成员的更新。

当前是平等协作模型：所有获得远端读写凭据和加密口令的成员都能查看、编辑、移动和删除全部共享书签。每台设备可以设置“设计组笔记本”“客厅电脑”等名称；名称随加密 payload 同步，用于冲突和删除来源显示，但不代表成员身份或权限。建议远端服务为每位成员提供独立应用密码或 SAS，以便单独撤销未来访问。已经下载到成员设备的数据无法远程收回。

静态网站地址本身不包含共享书签或凭据，普通访客只会获得自己的空数据库。团队库与私人书签不应混在同一个浏览器 Origin；需要同时使用时，可采用不同子域名或浏览器配置文件。项目保持纯前端，不提供中央账号、角色、审批或服务端审计。

创建第一份共享库时，由组织者使用普通“设置向导”配置远端并完成首次同步。其他成员使用“加入共享书签库”入口：向导固定使用远端模式，支持 WebDAV、Koofr 和 Azure Blob，并要求现有同步文件存在，默认开启自动上传和共享库更新检查，并引导填写成员凭据、统一加密口令及设备名称。如果当前浏览器已有本机书签，最后一步会明确显示数量并要求确认，避免把私人书签意外上传到共享库。

加入向导会执行真实读取、AES-GCM 解密、三方合并和 ETag/Hash 条件写入；失败时恢复原同步配置。密码、SAS Token 与加密口令仍只在内存中，除非成员明确选择仅在刷新本标签页时保留。

当前版本一次只支持一个同步位置。连接成功后，“加入共享书签库”和设置向导入口会隐藏，同步方式、远端地址和用户名变为只读，避免把“更换远端地址”误解成安全的书签库切换。如需使用另一套共享库，优先采用独立子域名或浏览器配置文件；确需重置时先移除当前同步配置，本机书签不会自动删除，之后加入其他库仍需明确确认是否参与合并。
...
默认情况下，所有书签只保存在当前浏览器的 IndexedDB 中，没有遥测或第三方网络请求。自动备份只会写入用户明确选择的本地目录；只有用户配置并主动解锁远端同步后，应用才会访问对应的同步地址。同步地址和用户名作为必要连接信息保存在本机 IndexedDB，密码和加密口令不进入长期设置。完整数据流见 [`docs/PRIVACY.md`](docs/PRIVACY.md)。
...
- 标签、URL 和同步地址规范化
- WebDAV、Koofr、Azure Blob 全球域名与 Azure 中国区域名识别
- WebDAV 条件版本探测、Koofr Hash、Azure Blob HEAD/ETag 探测和远端变化触发
- 多标签页远端检查锁与定时调度
