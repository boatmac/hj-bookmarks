...
- 三种恢复方式执行前都会在 `emergency/` 中写入一份当前数据，最多保留 10 份；紧急备份写入失败时不会修改 IndexedDB。
- 损坏、包含不安全 URL、无效父子层级或版本不兼容的文件会标记为不可恢复。
