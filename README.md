...
向导随后确认加密口令、自动同步和刷新凭据偏好，并在完成前执行一次真实的读取、写入和加密合并验证。Koofr、标准 WebDAV 及其浏览器兼容差异不会作为用户需要选择的同步类型；服务识别与协议适配封装在远端适配层中。尚未配置同步时，可以从右上角“备份/同步 → 同步设置向导”或同步设置窗口进入；连接成功后向导入口会隐藏，同步方式、远端地址和用户名会锁定。需要更换同步位置时必须先移除当前配置。
