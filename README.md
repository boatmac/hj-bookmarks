...
应用同时调用浏览器持久存储 API，尽量避免 IndexedDB 因存储压力被自动清理；是否授予由浏览器决定，直接打开本地文件时可能返回未授权。它与备份目录是两套独立机制：即使浏览器内部数据保护未获授权，已经写入 `bookmarks-latest.json` 和 `history/` 的文件也不会受影响。

### 从备份恢复

可以从右上角“备份/同步 → 从备份恢复”、自动备份设置窗口或空数据库首页进入恢复向导。重新选择原备份目录后，应用会逐一验证并列出最新备份、历史快照和恢复前紧急备份，显示时间、书签数、文件夹数及内容预览。

- **合并恢复**保留当前内容，只补回缺少的文件夹和书签；已有 URL 不重复导入。
- **替换恢复**将当前书签恢复为快照状态；执行前会在 `emergency/` 中写入一份当前数据，最多保留 10 份。
- 损坏、包含不安全 URL 或版本不兼容的文件会标记为不可恢复。
- 恢复记录会使用当前设备的新修改时间；被替换掉的项目会生成删除记录，使之后的加密同步能够安全传播恢复结果。
