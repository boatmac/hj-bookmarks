...
│   ├── help.js          页面内最终用户帮助
│   ├── backup-restore.js 备份发现、预览与安全恢复
...
恢复向导只接受 `format: bookmark-manager` 且版本受支持的 JSON，并在展示前验证结构、URL 和重复记录 ID。目录扫描逐文件让出主线程，快照内容只在选中时转换为恢复记录，避免同时保留所有历史文件的完整对象。

合并恢复复用导入去重与文件夹匹配逻辑；替换恢复通过 `storage.js` 中的单个 IndexedDB 事务重建书签，同时为快照中不存在的当前项目写入删除墓碑。所有恢复项目都会写入当前时间和当前设备 ID，以便下一次同步把恢复视为明确的本机修改。替换事务前必须先成功写入 `emergency/` 紧急备份。
